<template>
  <div class="about">
    <div class="filter">
      ...
      <button @click="filterStar">Star</button>
    </div>
    <div v-for="datas in row" :key="datas.id">
      <BoxSubmission
        :title="datas.title"
        :content="datas.thought"
        :date="datas.date"
        :number="datas.number"
        :star="datas.star"
        :id="datas.id"
        @update-to-db="changeValue"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    filterDate() {
      this.row = this.row.sort((a, b) => a - b);
    },
    filterStar() {
      const row = this.row;
      row.filter((obj) => obj.star == undefined).forEach((v) => (v.star = "0"));
      if (this.toggledStar) {
        this.row = row.sort((a, b) => (a.star < b.star ? 1 : -1));
        this.toggledStar = false;
      } else {
        this.row = row.sort((a, b) => (a.star > b.star ? 1 : -1));
        this.toggledStar = true;
      }
    },
    changeValue(star, id) {
      this.db
          ...
          tempDoc.forEach((element) => {
            let data = {
              date: this.timeFormat(
                element.date.seconds,
                element.date.nanoseconds
              ),
              title: element.title,
